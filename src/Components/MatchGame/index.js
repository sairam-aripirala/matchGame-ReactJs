...
class MatchGame extends Component {
  ...
  tick = () => {
    ...
    if (timeLeft > 1) {
      ...
      clearInterval(this.timerId)
      this.setState({isCompleted: true, timeLeft: 0})
    }
  }
  ...
  onCheckThumbnail = id => {
    ...
    const imageItem = imagesList.find(each => each.id === id)

    if (imageItem.id === selectedImageId) {
      this.setState(prev => ({
        score: prev.score + 1,
        selectedImageId:
  ...
  getContentSection = () => {
    const {isCompleted, selectedTabId, selectedImageId, score} = this.state

    const selectedImg = imagesList.find(each => each.id === selectedImageId)
    ...
    return (
      <div className="game-container">
        <img src={selectedImg.imageUrl} alt="match" className="main-image" />
        ...
        <ul className="thumbnails-list-container">
          {filteredThumbnailsList.map(eachImg => (
            <ThumbnailItem
              ...
              key={eachImg.id}
              onCheckThumbnail={this.onCheckThumbnail}
            />
          ))}
        </ul>
      </div>
  ...
  render() {
    ...
    const {timeLeft, score} = this.state
    ...
    return (
      <div className="bg-container">
        ...
        <div className="bottom-section">{contentSection}</div>
      </div>
    )
  }
}
